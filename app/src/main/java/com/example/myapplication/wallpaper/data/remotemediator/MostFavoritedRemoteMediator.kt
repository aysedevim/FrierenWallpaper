...
import com.example.myapplication.wallpaper.data.mapper.toMostFavoritedEntity
...
class MostFavoritedRemoteMediator(
    private val api: WallpaperApi,
    private val db: WallpaperDatabase,
    private val indexKey: String
) : RemoteMediator<Int, MostFavoritedEntity>() {

    private val label = "most_favorited:$indexKey"
    private val dao = db.mostFavoritedDao()
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, MostFavoritedEntity>
    ): MediatorResult {
        ...
        return try {
            val response = api.getMostFavorited(
                index = indexKey,
            ...
            db.withTransaction {
                ...
                val entities = response.mapIndexed { i, dto ->
                    dto.toMostFavoritedEntity(position = startPos + i)
                }
